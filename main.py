...
AD_MIN_SECONDS = max(1, int(os.environ.get("AD_MIN_SECONDS", "1")))
...
def telegram_chat_id(channel_id: str):
    try:
        return int(channel_id)
    except (TypeError, ValueError):
        return channel_id if str(channel_id).startswith("@") else f"@{channel_id}"


async def get_channel_presentation(item):
    """Resolve CA-style display name, avatar, and join link from Telegram."""
    channel_id = item["id"]
    chat = None
    try:
        chat = await telegram_bot_app.bot.get_chat(
            chat_id=telegram_chat_id(channel_id)
        )
    except Exception as exc:
        logger.warning("Could not load channel presentation for %s: %s", channel_id, exc)

    name = item["name"] or (
        getattr(chat, "title", None)
        or getattr(chat, "first_name", None)
        or f"Channel {int(item['index']) + 1}"
    )
    join_url = item["url"]
    if not join_url and chat is not None:
        if getattr(chat, "username", None):
            join_url = f"https://t.me/{chat.username}"
        elif getattr(chat, "invite_link", None):
            join_url = chat.invite_link
    if not join_url:
        join_url = await get_channel_invite_link(telegram_bot_app.bot, channel_id)

    image = item["image"]
    if not image and chat is not None and getattr(chat, "photo", None):
        image = f"/api/channel-photo?channel_id={quote(str(channel_id), safe='')}"

    return name, image, join_url


async def channel_photo(channel_id: str):
    """Proxy channel avatars so the bot token never appears in browser HTML."""
    cache_key = str(channel_id)
    cached = channel_photo_cache.get(cache_key)
    if cached and cached["expires_at"] > datetime.datetime.now(datetime.timezone.utc):
        return Response(
    ...
    try:
        ...
        if not photo:
            raise HTTPException(status_code=404, detail="Channel photo unavailable")
        file = await telegram_bot_app.bot.get_file(photo.small_file_id)
        content = bytes(await file.download_as_bytearray())
        channel_photo_cache[cache_key] = {
            "content": content,
        ...
        logger.warning("Could not load channel photo for %s: %s", channel_id, exc)
        raise HTTPException(status_code=404, detail="Channel photo unavailable")
...
async def build_join_state(token: str, user_id=None):
    ...
    for item, joined in zip(configs, memberships):
        name, image, join_url = await get_channel_presentation(item)
        channels.append({
            "index": item["index"],
            "id": item["id"],
            "name": name,
...
async def lifespan(app_instance: FastAPI):
    ...
    try:
        await telegram_bot_app.bot.set_my_name(name=BRAND_TITLE)
        await telegram_bot_app.bot.set_my_description(
            description="Secure protected-link access with Team Leader lectures."
        )
    except Exception as e:
        logger.warning(f"Could not sync Telegram bot title: {e}")
...
app.add_api_route("/api/channel-photo", channel_photo, methods=["GET"])
