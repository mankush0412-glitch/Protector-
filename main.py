...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await check_channel_membership(user_id, context):
        ...
        for ch in get_support_channels():
            ...
            if is_joined:
                channel_buttons.append(InlineKeyboardButton("𝙅𝙤𝙞𝙣𝙚𝙙 ✅", callback_data="already_joined"))
            else:
                pending_count += 1
                invite_link = await get_channel_invite_link(context, ch)
                channel_buttons.append(InlineKeyboardButton("📢 𝙅𝙤𝙞𝙣 𝘾𝙝𝙖𝙣𝙣𝙚𝙡", url=invite_link))
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "already_joined":
        await query.answer("✅ Already joined!", show_alert=False)
        return
...
async def lifespan(app_instance: FastAPI):
    ...
    required_vars = ["TELEGRAM_TOKEN", "RENDER_EXTERNAL_URL", "MONGODB_URI"]
